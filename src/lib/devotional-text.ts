...
const removableKoreanDevotionalLabels = [
...
function isMostlyEnglishOnly(text: string) {
  const compact = text.replace(/\s+/g, " ").trim();
  if (!compact || /[가-힣ㄱ-ㅎㅏ-ㅣ]/.test(compact)) return false;
  const letters = compact.match(/[A-Za-z]/g)?.length ?? 0;
  return letters >= 8 && englishOnlyPattern.test(compact);
}

function isLikelyTranslatedQuote(text: string) {
  const compact = text.replace(/\s+/g, " ").trim();
...
function isInlineEnglishQuoteWithTranslation(text: string) {
  const compact = text.replace(/\s+/g, " ").trim();
  if (compact.length > 700) return false;
  const englishLetters = compact.match(/[A-Za-z]/g)?.length ?? 0;
  if (englishLetters < 45) return false;
  const hasLocalizedTranslation = /[가-힣ㄱ-ㅎㅏ-ㅣก-๙]/.test(compact);
  if (!hasLocalizedTranslation) return false;
  return /[“”"']/.test(compact) || /[—–-]\s*[A-Za-z]/.test(compact);
}

export function stripKoreanGospelDevotionalExtras(body?: string | null) {
  if (!body) return "";
  ...
  for (const block of blocks) {
    ...
    // Gospel devotional policy: do not display source quotation excerpts
    // or their translated quotation blocks in the daily devotional body.
...
export function localizedGospelDevotionalBody(lang: string, body?: string | null) {
